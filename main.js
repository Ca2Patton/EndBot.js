...
client.on('guildMemberAdd', member => {
    ...
    if (!channel) return;
    channel.send(`Welcome to the Endless, ${member}! What's your PSN? What game are you here for? Use ``${prefix}(game) `` with game being the game you play! A list of games are: Destiny~~, Monster-Hunter, ~~and ESO.`);
});
...
client.on('message', message => {
    if (message.content ===`${prefix}destiny-join`) {
        ...
        channel.send("May or may not have added the role.")
    }
});
